...
- **공통 하단 컴포넌트**: 모든 세금 계산기는 최하단에 `FAQ`와 `TaxMoreCalculators` 컴포넌트를 반드시 포함하여 UI 일관성을 유지해야 함.

## 5. 입력 및 계산 가이드
- **세율 및 공제액 정보 명시**: 계산 시 적용되는 세율의 기준(연도 등) 및 각종 공제 적용 여부를 화면상에 안내할 것.
