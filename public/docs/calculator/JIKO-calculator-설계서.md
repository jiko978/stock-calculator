...
- `app/` : Next.js App 라우터 최상위 디렉토리 (랜딩 페이지 도메인: `jiko.kr`)
  ...
  - `calculator/` : 계산기 통합 라우트 (계산기 도메인: `jiko.kr/calculator`)
    ...
    - `stock/` : 주식 메뉴
      ...
      - `fee/` : 수수료 계산기 페이지  
    - `finance/`, `real-estate/`, `life/`, `health/`, `tax/` : 기타 카테고리별 디렉토리
...
금융(finance)
ㄴ [1차]예금 이자 : 예금이자율, 예금액, 예금기간별 예금이자 계산(네이버 참고)
ㄴ [1차]적금 이자 : 적금이자율, 적금액, 적금기간별 적금이자 계산(네이버 참고)
ㄴ [1차]대출 이자 : 대출이자율, 대출액, 대출기간별 대출이자 계산(네이버 참고)
